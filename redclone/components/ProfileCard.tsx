...
function ProfileCard () {
  ...
  const getPlayer = async () => {
    const res = await axios.get('/api/currentplay');
  ...
  */}
            ...
            <div className="text-slate-500 font-medium text-sm sm:text-base leading-tight truncate dark:text-slate-400"><Marquee pauseOnHover={true} gradientWidth="50" >{"Heelo!"}</Marquee></div>
            </div>
            </div>
            <button onClick={() => console.log(player)}>CLICK ME</button>
            </>
