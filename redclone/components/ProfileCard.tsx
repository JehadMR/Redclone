...
function ProfileCard () {
 ...
 const beforeDecimal = (num) => {
  ...
  const getProgress = () => {
    let seconds = beforeDecimal(result?.data?.progress_ms? result.data.progress_ms / 1000 : 0);
    let minutes = beforeDecimal(result?.data?.progress_ms? result.data.progress_ms / 60000 : 0 );
